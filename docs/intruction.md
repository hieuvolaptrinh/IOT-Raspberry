...
Tài liệu này hướng dẫn cách cấu hình và chạy file `ble_application.py` để board (Raspberry Pi) luôn tự động phát sóng Bluetooth (BLE). Từ đó, bạn có thể dùng điện thoại hoặc máy tính kết nối vào board và cấu hình mạng WiFi, API_URL, hoặc xem IP mạch bất cứ lúc nào.
...
## 💻 BƯỚC 2 — Cách hoạt động của `ble_application.py`
...
| Nhận dữ liệu VSL    | `abcf` | Write | Nhận chuỗi JSON kết quả dịch ngôn ngữ ký hiệu từ Flutter |
...
5. **Nhận kết quả VSL** (UUID `abcf`): Nhận JSON chứa mảng từ vựng (words) để mạch tự động tìm video và phát.
...
2. **Dán nội dung sau vào file:**
   ...
   > ⚠️ **Lưu ý:** Nếu bạn không dùng `lcd_env`, hãy trỏ lại thành `/usr/bin/python3`. Đảm bảo đường dẫn `/home/pi/IOT-Raspberry/ble_application.py` chính xác với vị trí file trên mạch của bạn.
...
### 📌 THÔNG TIN KẾT NỐI BLE (Ghi nhớ)
- **Tên thiết bị (Device Name):** `Pi-Setup`
- **Service UUID:** `00001234-0000-1000-8000-00805f9b34fb` (Trong nRF thường hiển thị tắt là `1234`)

**Các Characteristics trong Service `1234`:**
1. **[Write] Cấu hình WiFi:** `0000abcd-0000-1000-8000-00805f9b34fb` (Gửi tên WiFi + Pass)
2. **[Read] Xem IP Mạch:** `0000abce-0000-1000-8000-00805f9b34fb` (Lấy IP để kết nối từ xa)
3. **[Write] Gửi dữ liệu VSL:** `0000abcf-0000-1000-8000-00805f9b34fb` (Gửi chuỗi từ vựng cho mạch phát video)
...
4. Vào tab **Client**, tìm Service có UUID là **`1234`** và bấm vào để xổ xuống danh sách các Characteristic.
